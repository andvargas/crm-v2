...
type Draft = { fullName: string; jobTitle: string; email: string; phone: string; mobile: string; contactType: string; businessRole: string; connectionType: string; notes: string; company: string };
const emptyDraft: Draft = { fullName: "", jobTitle: "", email: "", phone: "", mobile: "", contactType: "", businessRole: "", connectionType: "", notes: "", company: "" };
...
export default function ContactDetailPage() {
  ...
  const [interactionDraft, setInteractionDraft] = useState({ type: "enquiry", channel: "email", status: "new", note: "", outcome: "" });
  ...
  const addInteraction = useMutation({
    ...
    onSuccess: () => {
      ...
      setInteractionDraft({ type: "enquiry", channel: "email", status: "new", note: "", outcome: "" });
    },
  ...
  const openEditor = () => {
    if (!contact) return;
    setDraft({ fullName: contactName(contact), jobTitle: contact.jobTitle || "", email: contact.email || "", phone: contact.phone?.generic || contact.phone?.office || "", mobile: contact.phone?.mobile || "", contactType: contact.contactType || "", businessRole: contact.businessRole || "", connectionType: BUSINESS_ROLE_VALUES.has(contact.connectionType || "") ? "" : contact.connectionType || "", notes: contact.notes || "", company: companyId(contact) || "" });
    setFormError("");
    setEditing(true);
  };
  ...
  const submit = (event: FormEvent) => {
    ...
    updateContact.mutate({ name: { fullName: draft.fullName.trim() }, jobTitle: draft.jobTitle.trim(), email: draft.email.trim(), phone: { generic: draft.phone.trim(), mobile: draft.mobile.trim() }, contactType: draft.contactType, businessRole: draft.businessRole, connectionType: draft.connectionType, notes: draft.notes.trim(), company: draft.company || null });
  };
...
function SelectField({ label, value, change, options, allowEmpty = false }: { label: string; value: string; change: (value: string) => void; options: { value: string; label: string }[]; allowEmpty?: boolean }) { return <label><span className="mb-1.5 block text-xs font-semibold text-slate-600">{label}</span><select value={value} onChange={(event) => change(event.target.value)} required={!allowEmpty} className="h-11 w-full rounded-xl border border-slate-200 px-3 text-sm"><option value="" disabled={!allowEmpty}>{allowEmpty ? "None" : "Select…"}</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>; }
