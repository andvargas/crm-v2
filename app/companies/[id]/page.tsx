...
export default function CompanyDetailPage() {
  ...
  const submit = (event: FormEvent) => {
    ...
    updateCompany.mutate({
      ...
      phone: draft.phone.trim() ? [draft.phone.trim()] : [], website: draft.website.trim(),
  ...
  return <CrmShell activePath="/companies"><div className="mx-auto max-w-[1350px] px-4 py-7 md:px-8 md:py-9">
    ...
    <section className="mt-7 grid gap-5 xl:grid-cols-[.78fr_1.4fr]">
      <div className="space-y-5"><article className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-bold text-slate-950">Company details</h2><div className="mt-5 space-y-4"><Detail icon={Mail} label="Email" value={company.genericEmail} href={company.genericEmail ? `mailto:${company.genericEmail}` : undefined} /><Detail icon={Phone} label="Phone" value={company.phone?.[0]} /><Detail icon={ExternalLink} label="Website" value={company.website} href={company.website} /><Detail icon={UsersRound} label="Staff" value={company.staffNumber?.toLocaleString()} /><Detail icon={Building2} label="Segment" value={company.segment} /></div></article>
      </div>
