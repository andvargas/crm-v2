...
import { ArrowUpRight, CalendarClock, ChevronDown, ChevronLeft, ChevronRight, FilterX, Link2, Mail, MessageSquarePlus, Pencil, Phone, Radio, SearchCheck, UserPlus, X } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { AsyncCombobox } from "../../components/async-combobox";
import { CrmShell } from "../../components/crm-shell";
import { InteractionEditor } from "../../components/interaction-editor";
import { ActivityContactModal } from "../../components/activity-contact-modal";
...
export default function ActivitiesPage() {
  const initialActivity = typeof window === "undefined" ? "" : new URLSearchParams(window.location.search).get("activity") || "";
  ...
  const [targetActivity, setTargetActivity] = useState(initialActivity);
  const [expanded, setExpanded] = useState<string | null>(initialActivity || null);
  ...
  const params = useMemo(() => {
    ...
    if (targetActivity) values.set("activity", targetActivity);
  ...
  }, [channel, company.value, contact.value, debouncedSearch, from, page, status, targetActivity, to, type]);
  ...
  const clearFilters = () => { setSearch(""); setType(""); setChannel(""); setStatus(""); setCompany({ value: "", label: "" }); setContact({ value: "", label: "" }); setFrom(""); setTo(""); setTargetActivity(""); setExpanded(null); setPage(1); window.history.replaceState({}, "", "/activities"); };
  ...
  return <><CrmShell activePath="/activities" search={search} onSearch={(value) => { setSearch(value); setPage(1); }}><div className="mx-auto max-w-[1500px] px-4 py-7 md:px-8 md:py-9">
    ...
    {targetActivity && <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-3"><p className="text-sm font-semibold text-indigo-800">Showing the activity opened from your notification.</p><button type="button" onClick={clearFilters} className="rounded-lg bg-white px-3 py-2 text-xs font-bold text-indigo-700 shadow-sm">Show all activities</button></div>}
      ...
      {query.isLoading ? <p className="p-12 text-center text-sm text-slate-500">Loading activities…</p> : query.isError ? <p className="p-12 text-center text-sm text-rose-600">Activities could not be loaded.</p> : data?.items.length ? <div className="divide-y divide-slate-100">{data.items.map((activity) => {
        ...
        return <article key={activity._id}>
          ...
          {isExpanded && <div className="border-t border-slate-100 bg-slate-50/70 px-5 py-5">
            ...
            {activity.contact && typeof activity.contact === "object" && <ActivityContactCard contact={activity.contact} company={companyLabel(activity)} />}
...
function ActivityContactCard({ contact, company }: { contact: Exclude<Interaction["contact"], string | undefined>; company?: string }) {
  const name = contact.name?.fullName || [contact.name?.firstName, contact.name?.lastName].filter(Boolean).join(" ") || "Contact";
  const phone = contact.phone?.mobile || contact.phone?.direct || contact.phone?.generic || contact.phone?.office || contact.phone?.home;
  return <section className="mb-5 rounded-xl border border-slate-200 bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-wider text-indigo-600">Contact details</p><h3 className="mt-1 text-base font-bold text-slate-900">{name}</h3><p className="mt-0.5 text-xs text-slate-500">{[contact.jobTitle, company].filter(Boolean).join(" · ") || "Linked contact"}</p></div><a href={`/contacts/${contact._id}`} className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">Open contact<ArrowUpRight size={14} /></a></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{phone ? <a href={`tel:${phone.replace(/[^\d+]/g, "")}`} className="flex min-w-0 items-center gap-3 rounded-xl bg-emerald-50 px-3.5 py-3 text-emerald-800"><Phone size={17} className="shrink-0" /><span className="truncate text-sm font-semibold">{phone}</span></a> : <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-3.5 py-3 text-slate-400"><Phone size={17} /><span className="text-sm">No phone number</span></div>}{contact.email ? <a href={`mailto:${contact.email}`} className="flex min-w-0 items-center gap-3 rounded-xl bg-indigo-50 px-3.5 py-3 text-indigo-800"><Mail size={17} className="shrink-0" /><span className="truncate text-sm font-semibold">{contact.email}</span></a> : <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-3.5 py-3 text-slate-400"><Mail size={17} /><span className="text-sm">No email address</span></div>}</div></section>;
}
...
function FollowUpModal({ interaction, onClose, onSaved }: { interaction: Interaction; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="fixed inset-0 z-[170] flex items-end justify-center bg-slate-950/45 p-2 sm:items-center sm:p-4">
    <div role="dialog" aria-modal="true" aria-labelledby="follow-up-title" className="flex max-h-[calc(100dvh-1rem)] w-full max-w-md flex-col overflow-hidden rounded-2xl bg-white shadow-2xl sm:max-h-[calc(100dvh-2rem)]">
      <div className="flex shrink-0 items-start justify-between border-b border-slate-100 px-5 py-4 sm:px-6 sm:py-5"><div><h2 id="follow-up-title" className="text-lg font-bold text-slate-950">Follow up</h2><p className="mt-1 text-xs text-slate-500">Set a reminder for {label}.</p></div><button type="button" onClick={onClose} aria-label="Close" className="rounded-lg p-2 text-slate-400 hover:bg-slate-100"><X size={18} /></button></div>
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5 sm:p-6"><label><span className="mb-1.5 block text-xs font-semibold text-slate-600">Reminder date and time</span><input type="datetime-local" required value={date} onChange={(event) => setDate(event.target.value)} className="h-12 w-full rounded-xl border border-slate-200 px-3.5 text-base outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100" /></label><label><span className="mb-1.5 block text-xs font-semibold text-slate-600">Reminder note <span className="font-normal text-slate-400">(optional)</span></span><textarea value={note} onChange={(event) => setNote(event.target.value)} rows={3} placeholder="What needs to happen next?" className="w-full rounded-xl border border-slate-200 p-3 text-base outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100" /></label>{error && <p className="text-sm text-rose-600">{error}</p>}</div>
      <div className="flex shrink-0 flex-col-reverse gap-2 border-t border-slate-100 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">{interaction.followUpAt ? <button type="button" disabled={saving} onClick={clear} className="rounded-xl px-4 py-3 text-sm font-semibold text-rose-600 disabled:opacity-50">Remove reminder</button> : <span />}<div className="grid grid-cols-2 gap-2 sm:flex"><button type="button" onClick={onClose} className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold">Cancel</button><button type="button" disabled={saving || !date} onClick={save} className="rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{saving ? "Saving…" : "Save follow-up"}</button></div></div>
    </div>
  </div>;
}
