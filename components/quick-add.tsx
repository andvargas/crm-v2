...
export function QuickAdd() {
  ...
  const [form, setForm] = useState({ name: "", email: "", jobTitle: "", phone: "", address: "", industry: "", website: "", company: "", companyName: "", contact: "", contactName: "", type: "enquiry", channel: "email", status: "new", note: "", outcome: "" });
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setForm({ name: "", email: "", jobTitle: "", phone: "", address: "", industry: "", website: "", company: "", companyName: "", contact: "", contactName: "", type: "enquiry", channel: "email", status: "new", note: "", outcome: "" });
    },
  ...
  const submit = (event: FormEvent) => {
    ...
    if (kind === "contact") return mutation.mutate({ path: "/contacts/add", body: { name: { fullName: form.name.trim() }, email: form.email.trim(), jobTitle: form.jobTitle.trim(), phone: { generic: form.phone.trim() }, address: { formatted: form.address.trim() }, ...(form.company ? { company: form.company } : {}) } });
    mutation.mutate({ path: "/interactions/add", body: {
      ...
      phone: form.phone.trim(), email: form.email.trim(), physicalAddress: form.address.trim(),
  ...
  const modal = kind && <div className="fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-slate-950/45 p-4"><div role="dialog" aria-modal="true" aria-labelledby="quick-add-title" className="my-8 w-full max-w-xl rounded-2xl bg-white text-slate-900 shadow-2xl"><div className="flex items-center justify-between border-b border-slate-100 px-6 py-5"><div><h2 id="quick-add-title" className="text-lg font-bold">Add {kind}</h2><p className="mt-1 text-xs text-slate-500">Create a new CRM record.</p></div><button aria-label="Close" onClick={() => setKind(null)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100"><X size={19} /></button></div><form onSubmit={submit} className="p-6"><div className="grid gap-4 sm:grid-cols-2">
      ...
      {kind === "contact" && <><Field label="Full name" value={form.name} change={(name) => setForm({ ...form, name })} required /><Field label="Job title" value={form.jobTitle} change={(jobTitle) => setForm({ ...form, jobTitle })} /><Field label="Email" type="email" value={form.email} change={(email) => setForm({ ...form, email })} /><Field label="Phone" type="tel" value={form.phone} change={(phone) => setForm({ ...form, phone })} /><AsyncCombobox label="Company (optional)" endpoint="/companies/lookup" value={form.company} selectedLabel={form.companyName} emptyLabel="Standalone contact" placeholder="Search companies…" onChange={(option) => setForm({ ...form, company: option?.value || "", companyName: option?.label || "" })} /><TextArea label="Address" value={form.address} change={(address) => setForm({ ...form, address })} /></>}
      {kind === "interaction" && <><AsyncCombobox label="Contact (optional)" endpoint="/contacts/lookup" value={form.contact} selectedLabel={form.contactName} emptyLabel="No contact" placeholder="Search contacts…" onChange={chooseContact} /><AsyncCombobox label="Company (optional)" endpoint="/companies/lookup" value={form.company} selectedLabel={form.companyName} emptyLabel="No company" placeholder="Search companies…" onChange={(option) => setForm({ ...form, company: option?.value || "", companyName: option?.label || "" })} /><Field label="Name when no contact" value={form.name} change={(name) => setForm({ ...form, name })} /><Select label="Type" value={form.type} change={(type) => setForm({ ...form, type })} options={INTERACTION_TYPES.map((value) => ({ value, label: value.replaceAll("-", " ") }))} /><Select label="Channel" value={form.channel} change={(channel) => setForm({ ...form, channel })} options={INTERACTION_CHANNELS.map((value) => ({ value, label: value.replaceAll("-", " ") }))} /><Select label="Status" value={form.status} change={(status) => setForm({ ...form, status })} options={LEAD_STATUSES} />{(form.channel === "phone" || form.type === "phonecall" || form.phone) && <Field label="Phone number" type="tel" value={form.phone} change={(phone) => setForm({ ...form, phone })} />}{(["email", "website-form"].includes(form.channel) || form.email) && <Field label="Email address" type="email" value={form.email} change={(email) => setForm({ ...form, email })} />}{(form.channel === "in-person" || form.address) && <TextArea label="Physical address" value={form.address} change={(address) => setForm({ ...form, address })} />}<Field label="Note" value={form.note} change={(note) => setForm({ ...form, note })} /><label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-semibold text-slate-600">First message</span><textarea value={form.outcome} onChange={(event) => setForm({ ...form, outcome: event.target.value })} rows={4} className="w-full rounded-xl border border-slate-200 p-3 text-sm outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100" /></label></>}
...
function TextArea({ label, value, change }: { label: string; value: string; change: (value: string) => void }) { return <label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-semibold text-slate-600">{label}</span><textarea value={value} onChange={(event) => change(event.target.value)} rows={3} className="w-full rounded-xl border border-slate-200 p-3 text-sm outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100" /></label>; }
