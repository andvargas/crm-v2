...
function Modal({ interaction, mode, onClose }: { interaction: Interaction; mode: Mode; onClose: () => void }) {
  ...
  const [phone, setPhone] = useState(interaction.phone || "");
  const [email, setEmail] = useState(interaction.email || "");
  const [address, setAddress] = useState(interaction.physicalAddress || "");
  ...
  const matchedCompany = companyMatches.data?.items.find((item) => item.label.trim().toLocaleLowerCase() === activityCompanyName.trim().toLocaleLowerCase());
  const effectiveCompany = company || matchedCompany || null;
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const created = await api<Contact>("/contacts/add", { method: "POST", body: JSON.stringify({
        name: { fullName: name.trim() }, phone: { generic: phone.trim() }, email: email.trim(), address: { formatted: address.trim() }, ...(effectiveCompany ? { company: effectiveCompany.value } : {}),
      }) });
      return api<Interaction>(`/interactions/${interaction._id}`, { method: "PATCH", body: JSON.stringify({
        contact: created._id, fullName: name.trim(),
        ...(effectiveCompany ? { company: effectiveCompany.value, companyName: effectiveCompany.label } : {}),
      }) });
    },
  ...
  return createPortal(<div className="fixed inset-0 z-[160] grid place-items-center overflow-y-auto bg-slate-950/45 p-4"><div role="dialog" aria-modal="true" className="my-8 w-full max-w-lg rounded-2xl bg-white shadow-2xl">
    ...
    <form onSubmit={submit} className="space-y-4 p-6">{mode === "create" ? <><Field label="Contact name" value={name} onChange={setName} /><div className="grid gap-4 sm:grid-cols-2"><Field label="Phone number" type="tel" value={phone} onChange={setPhone} required={false} /><Field label="Email address" type="email" value={email} onChange={setEmail} required={false} /></div><label><span className="mb-1.5 block text-xs font-semibold text-slate-600">Physical address</span><textarea value={address} onChange={(event) => setAddress(event.target.value)} rows={3} className="w-full rounded-xl border border-slate-200 p-3 text-sm outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100" /></label><AsyncCombobox label="Company" endpoint="/companies/lookup" value={effectiveCompany?.value || ""} selectedLabel={effectiveCompany?.label} emptyLabel={activityCompanyName ? `No exact match for ${activityCompanyName}` : "No company"} placeholder="Search companies…" onChange={setCompany} />{activityCompanyName && !effectiveCompany && <p className="text-xs text-amber-700">Activity company: {activityCompanyName}. Search and select it if it already exists.</p>}</> : <AsyncCombobox label="Contact" endpoint="/contacts/lookup" value={contact?.value || ""} selectedLabel={contact?.label} emptyLabel="Select a contact" placeholder="Search contacts…" onChange={setContact} />}
...
function Field({ label, value, onChange, type = "text", required = true }: { label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean }) {
  return <label><span className="mb-1.5 block text-xs font-semibold text-slate-600">{label}</span><input type={type} value={value} onChange={(event) => onChange(event.target.value)} required={required} className="h-11 w-full rounded-xl border border-slate-200 px-3.5 text-sm outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100" /></label>;
}
