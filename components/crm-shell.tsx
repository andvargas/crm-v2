...
export function CrmShell({ children, activePath, search, onSearch }: { children: React.ReactNode; activePath: string; search?: string; onSearch?: (value: string) => void }) {
  ...
  return <div className="min-h-screen max-w-full overflow-x-hidden bg-[#f7f8fb] text-slate-900">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[272px] flex-col border-r border-slate-200 bg-white transition-transform print:hidden lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <nav className="flex-1 overflow-y-auto px-3 pb-6"><p className="px-3 pb-2 pt-5 text-[11px] font-bold uppercase tracking-[.14em] text-slate-400">Workspace</p>{workspace.map((item) => <NavLink key={item.href} item={item} active={activePath} />)}<p className="px-3 pb-2 pt-7 text-[11px] font-bold uppercase tracking-[.14em] text-slate-400">Finance</p>{finance.map((item) => <div key={item.href}><NavLink item={item} active={activePath} />{item.href === "/budget" && activePath.startsWith("/budget") && <div className="mb-2 ml-6 border-l border-slate-200 pl-3"><Link href="/budget" className={`block rounded-lg px-3 py-2 text-xs font-semibold ${activePath === "/budget" ? "bg-indigo-50 text-indigo-700" : "text-slate-500 hover:bg-slate-50"}`}>Transactions</Link><Link href="/budget/cashflow" className={`mt-0.5 block rounded-lg px-3 py-2 text-xs font-semibold ${activePath === "/budget/cashflow" ? "bg-indigo-50 text-indigo-700" : "text-slate-500 hover:bg-slate-50"}`}>Cashflow details</Link></div>}</div>)}</nav>
